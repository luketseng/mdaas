...
class mdaas_tools():
    ...
    def _get_mdaas_data(self, url):
        try:
            response = requests.get(url, verify=False, timeout=10)#DEV 
    ...
    def check_UUT_alived(self, sys_info, path, act):
        ...
        cmd="sshpass -p 'msft' ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 root@{} 'journalctl -a' > /opt/logs/{}/journalctl.log".format(ip, sn) # DEV 
        ...
        if match:
            ...
            with open(status_path, 'r') as f:
                ...
                mdaas_mode=None if not json_result.get('detail', None) else json_result.get('detail').get('mdaas_mode', None) # Waid DEV
                if mdaas_mode == 'gen_configs':
                    print('Blade is still in gen config step-> remove sys-info to do power cycle for next run')
                    os.remove(path)
                    return False
                #print(repr(json_result), type(json_result))
                detail = json_result.get('detail', {})
                if detail:
            ...
            L10_mdaas_mode_list=['bsl', 'mfg_l10_offline_a', 'mfg_l10_offline_c']
            ...
            #print(test_result, mdaas_mode, 'SHC test executed' in str(error_list.values()), list(error_list.values()))
            if test_result=='passed' and not self.err_flag:
                ...
                with open(unit_error_list_path, 'w+') as fp:
                    fp.write(req)
                    fp.close()
            elif test_result=='passed' and mdaas_mode in L10_mdaas_mode_list:
                self.pass_cnt += 1
                self.mod_UUT_stat(sn, 'ResultPass')
            #elif test_result=='failed' and mdaas_mode == 'intelshc' and 'SHC test executed' in str(error_list.values()):
            #    self.pass_cnt += 1
            ...
            if test_result=='failed':
                ...
                #os.remove(path)
                #return False
            if mdaas_mode in SHC_mdaas_mode_list and (test_result=='failed' or self.err_flag):
                print('------- Get fail in MD stage')
                timestamp = test_start_time
                path = "/opt/logs/{SN}-{STAGE}-{T}.zip".format(SN=sn, STAGE=stage, T=timestamp)
                if not os.path.exists(path):
                    tools.get_cmd_req('curl -X POST "http://127.0.0.1:9796/api/v1/mdaas/download" -H "accept: application/json" -H "Content-Type: application/json" -d \'{{"ip": "{}", "sn":"{}"}}\' -o {}'.format(ip, sn, path))
                else:
    ...
    def check_rk_alived(self, sn, *args, **kwargs):
        ...
        regex = re.compile(r"CSN.*'(.*)',.*CPN.*RACK MANAGER MAC")
...
if __name__ == '__main__':
    ...
    push_amd_to_next = False #Waid DEV
        ...
        if sys_file_count != exp_sys_count:
            ...
            for i in unit_region:
                ...
                if not os.path.exists(sys_file_path):
                    rk_conn.sendline('show system info -i {}'.format(i))
                    ...
                    if match and match.group(1) == 'Success':
                        total_cnt += 1
                        # Waid DEV: for AMD rack , should skip to next stage for MD 
                        if model in ['C2062']:
                            amd_blade_cnt += 1
                            print('\t this blade is AMD')
                            if rk_stage != 'MD': 
                                # power cycle
                                print('Power cycle this blade:%s' % i)
                        ...
                        #print(req)
                        match = re.search(r'Mac Address: (.*)\r', req)
                        mac = match.group(1) if match else None
                        print(mac)
                        mac = net_lib.EUI_mac(mac, formatting=mac_unix_expanded)
                        mac = mac.replace(':', '-')
                        dst = os.path.join(tools.tftp_path, "{}_menu.ipxe".format(mac))
                        if rk_stage == 'M1':
                            if os.path.isfile(dst):
                                print("remove {}".format(dst))
                                cmd = "rm {}".format(dst)
                                os.system(cmd)
                            print("create {} with L10 image".format(dst))
                            cmd = "ln -s {} {}".format("base_menu_l10.ipxe", dst)
                            os.system(cmd)
                        else:
                            if os.path.isfile(dst):
                                print("remove {}".format(dst))
                                cmd = "rm {}".format(dst)
                                os.system(cmd)
                            print("create {} with shc image".format(dst))
                            cmd = "ln -s {} {}".format("base_menu_shc.ipxe", dst)
                            os.system(cmd)

>>>>>>> 807764c4e4069966a8ce444cdd68ce0a3e034718
                        # AC Cycle sys if file not exist
                            ...
                            print('------- AC Cycle sys_info-{}'.format(i))
                            command='set manager port off -i {}'.format(i)
                            rk_conn.sendline(command)
                            index3 = rk_conn.expect(['WcsCli#', pexpect.TIMEOUT])
                            if index3 == 0:
                                print(rk_conn.before.decode())
                                time.sleep(5)
                            elif index3 == 1:
                            ...
                            command='set manager port on -i {}'.format(i)
                            rk_conn.sendline(command)
                            index4 = rk_conn.expect(['WcsCli#', pexpect.TIMEOUT])
                            if index4 == 0:
                                print(rk_conn.before.decode())
                                #print('wait 60 sec')
                                #time.sleep(60)
            ...
            #Waid DEV
            print('\tAMD blade count:%s' % amd_blade_cnt)
            print('\tTotal blade count:%s '% total_cnt)
            if (amd_blade_cnt == total_cnt) and rk_stage == 'MD':
                push_amd_to_next = True
                continue
    ...
    for k,v in tools.rack_maps.items():
        ...
        if push_amd_to_next or tools.pass_cnt == e_pass_cnt:# Waid DEV
        #if True:
            # get current stage
            tools.check_RK_stage(rk_sn, rk_index=k)
            stage = tools.rack_maps[k].get('STAGE', None)
            ...
            for x,y in v['UUT'].items():
                ...
                tools.mod_UUT_stat(y['SN'], 'ResultPass')
                # upload USN info
                ...
                #    soc_ip=y['SOC_IP']
                #    cmd="sshpass -p overlake ssh -o StrictHostKeyChecking=no ovl@{} 'sudo -S <<< 'overlake' rm CP_autoReconfig.sh /vol/data/persistent/tests/fpga.py /vol/data/persistent/tests/systemd/SoCFPGATestSvc.service'".format(soc_ip)
                #    print(cmd)
            ...
            if stage == 'MD':
                ...
                if match:
                    ...
                    if os.path.exists(rk_sn_path):# and n_stage == 'M1':
                        try:
                            shutil.rmtree(rk_sn_path)
                            tools.create_links(v) #TODO~~~ it might not needed if in M1, after rmtree it will create grub link
                        except OSError as e:
                            print("Error: %s - %s." % (e.filename, e.strerror))
                else:
