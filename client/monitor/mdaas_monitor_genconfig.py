...
class mdaas_tools():
    ...
    def _download_api(self, url): # Waid
        try:
            print('===== Downloading GOLDEN CONFIG FILE.......' )
            ...
            print('===== GOLDEN CONFIG FILE:%s download' % file_path)
            return fname
    ...
    def check_UUT_alived(self, sys_info, sys_info_path): # Waid
        sn = sys_info['SN']
        ...
        if not os.path.isfile(sys_info_path):
            ...
            print('------- file:%s not find: modify UUT status'% sys_info_path)
        ...
        if test_result == 'finished' and not self.history_has_err(result) and mdaas_mode == 'gen_configs':
            ...
            print('======= ID: [{}] SN:[{}] IP:[{}] gen config fail'.format(Id, sn, ip))
    ...
    def create_soft_link(self, rk_info):
        ...
        print("------- RUN create_soft_link(rk_info) stage=%s" % stage)
    ...
    def polling_status(self, rk_info, stage): # Waid
        ...
        for k,v in rk_info.get('UUT', {}).items():
            ...
            # Check UUT alived
            if not self.check_UUT_alived(v, sys_info_file):
    ...
    def _download_blade_golden_conf(self, blade_info, gconfig_files, cnt): # Waid
        ...
        for fname in gconfig_files:
            ...
            if ppn in fname and bpn in fname:
                cnt += 1
                print('==== ALREADY EXIST(PPN:%s, BPN:%s)... continue' % (ppn, bpn))
                return cnt
        if not ip or not ping(ip):
            print('===== FAIL to PING...blade id:%s, mac:%s, IP:%s' % (blade_id, mac, ip))
            return cnt
        # get status API and save to status file
    ...
    def download_golden_conf(self, rk_info):
        print("----- RUN download golden config function")
        ...
        if pass_perc < DWNLOAD_THRESHOLD:
            print("download percent: %s " % pass_perc)
            return False
        return True
...
if __name__ == '__main__':
    ...
    for k,v in rack_maps.items():
        ...
        if not tools.get_RK_GLB(rk_sn, rk_index=k):
            continue
        # ++++++++++++++ define new stage(GD) for pre-test control disable other stage
    ...
    for k,v in tools.rack_maps.items():
        ...
        if tools.pass_cnt == e_pass_cnt:# and match
            ...
            #continue
            #req = tools.get_cmd_req('python3 {} -s {}'.format(tools.complete_py, rk_sn)) # Waid DEV
            req = tools.get_cmd_req('python3 %s -s %s -t %s' % (tools.complete_py, rk_sn, stage))
            req = "CompleteResult : OK"
            ...
            print('\tSFCS stage:%s, original stage:%s' % (n_stage, stage))
